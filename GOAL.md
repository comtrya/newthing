...
- [x] Implement repo home layout — *repo home now resolves `/r/:groups+/:repo` through `workspace.repositoryByPath`, renders repository identity, mounts `repository.overview/code/issues/checks` only after a real repository context is available, and passes repository id/groups/name/path into mounted custom elements before connection; `bun run typecheck:v3`, `bun run build:v3`, repo slot smoke, root frontend checks, and `git diff --check` pass.*
- [x] Implement extension route layout (`/x/<prefix>/...`) that mounts the registered route element — *added an SDK route registry with prefix/path matching, `/x/:prefix/:rest*` now resolves the matching route and appends its custom element with `routeParams` set before connection, and route smoke covers `/`, `/:workspaceId/:number`, misses, and subscriber notification; `bun run typecheck:v3`, `bun run build:v3`, root frontend checks, and `git diff --check` pass.*
- [x] Wire `defineResourceCardElement()`, `defineInlineEditElement()`, `defineSkeletonElement()` at startup — *shell-v3 startup now registers the three SDK custom elements before mounting Vue; a happy-dom smoke confirms all three tags are registered, and `bun run typecheck:v3`, `bun run build:v3`, root frontend checks, and `git diff --check` pass.*
- [x] Wire `bindGlobalShortcut()` for the command palette + a default palette UI — *shell-v3 now binds the SDK Cmd/Ctrl-K shortcut, registers core navigation commands, and renders a Vue command palette backed by `listCommands`/`filterCommands`; happy-dom shortcut/filter smoke, `bun run typecheck:v3`, `bun run build:v3`, root frontend checks, and `git diff --check` pass.*
- [x] Wire `subscribeLiveEvents()` to the kernel's SSE stream — *SDK live events now use the real `/events/session` + `/events?session=...` SSE flow, normalize kernel CloudEvent frames, and shell-v3 subscribes in the topbar live status; mocked SSE smoke, `bun run typecheck:v3`, `bun run build:v3`, root frontend checks, and `git diff --check` pass.*
- [x] Dev server starts; `/r/rawkode/rawkode` renders a Vue page that mounts at least one custom element — *Fresh backend started from `/private/tmp/comtrya-m7-fresh-rawkode` on `127.0.0.1:8081`, Vite shell dev server started on `127.0.0.1:5175` with `COMTRYA_SERVER_URL=http://127.0.0.1:8081`, and the seeded `rawkode/rawkode` route resolved through GraphQL as `repo_01HV0K4XAVE2H6R5M8KJZ8R4W1`. Headless Chrome verified `http://127.0.0.1:5175/r/rawkode/rawkode` rendered title `Comtrya Shell v3`, no Vite overlay, no relevant console warnings/errors, four `repo-slot-*` frames, and the real `repository.overview` contribution mounted one `comtrya-repository-summary` custom element. Authenticated localStorage token smoke also verified live events reach `live` without console errors. `cargo test -p comtrya-server graphql_workspace_repository_by_path_resolves_rawkode_smoke_repo`, `bun run typecheck:v3`, `bun run build:v3`, root `bun run typecheck`, root `bun run build`, and `git diff --check` pass; final adversarial re-review reported no CRITICAL/IMPORTANT/NIT findings.*

## M8 — Extension UI in Vue
- [x] `ext_issues`: rewrite `ui/` into Vue SFCs (`issues-list`, `issue-detail`, `issue-card`) - *added typed Vue SFC source for `IssueCard`, `IssuesList`, and `IssueDetail` plus shared GraphQL helpers/types and a custom-element host bridge for linked epics/comments; preserved the existing issue card/detail/list smoke hooks and GraphQL behavior while leaving `defineExtensionWidget` registration and bundle emission for the next checkbox. `vue-tsc --noEmit -p extensions/first-party/ext_issues/ui/tsconfig.json`, `bun run typecheck:v3`, `bun run build:v3`, root `bun run typecheck`, and `git diff --check` pass.*
- [x] `ext_issues`: register via `defineExtensionWidget`; bundler emits the JS bundle - *added `ui/src/register.ts`, `ui/package.json`, and a Vite custom-element build that emits `extensions/first-party/ext_issues/assets/index.js`; the bundle registers `comtrya-issue-card`, `comtrya-issues-list`, `comtrya-issues-repo-list`, and `comtrya-issue-detail` through `defineExtensionWidget`, preserves the default `setup(host)` export, and keeps legacy `card.ref` working through the Vue SDK alias hook. `extensions/bundler/build-extension.sh extensions/first-party/ext_issues` now builds the UI bundle and refreshes `ui/manifest.json` `entryIntegrity`; the emitted bundle imports without `process.env` leaks, the manifest hash matches `shasum -a 256`, the happy-dom bundle smoke verifies light-DOM card/detail smoke hooks plus client forwarding, and `vue-tsc --noEmit -p extensions/first-party/ext_issues/ui/tsconfig.json`, `bun run typecheck:v3`, `bun run build:v3`, root `bun run typecheck`, root `bun run build`, and `git diff --check` pass.*
- [x] `ext_epics`: same - *rewrote the epics card/list/detail UI as Vue SFCs, added `ui/src/register.ts` with `defineExtensionWidget` registrations for `comtrya-epic-card`, `comtrya-epics-board`, `comtrya-epics-index`, and `comtrya-epic-detail`, preserved the default `setup(host)` export and the legacy `card.ref` alias, and emitted `extensions/first-party/ext_epics/assets/index.js` through `extensions/bundler/build-extension.sh extensions/first-party/ext_epics` with refreshed `ui/manifest.json` integrity. The emitted bundle imports without `process.env` leaks, the manifest hash matches `shasum -a 256`, the happy-dom bundle smoke verifies light-DOM card/detail smoke hooks plus linked issue/comment client forwarding, and `vue-tsc --noEmit -p extensions/first-party/ext_epics/ui/tsconfig.json`, `bun run typecheck:v3`, `bun run build:v3`, root `bun run typecheck`, root `bun run build`, and `git diff --check` pass.*
- [x] `ext_pull_requests`: same - *ported the existing placeholder pull-request queue/detail/slot widgets to Vue SFCs, registered `comtrya-pulls-queue`, `comtrya-pulls-detail`, `comtrya-pulls-your-work`, and `comtrya-pulls-overview` through `defineExtensionWidget`, preserved the default `setup(host)` slots/routes, and emitted `extensions/first-party/ext_pull_requests/assets/index.js` through `extensions/bundler/build-extension.sh extensions/first-party/ext_pull_requests` with refreshed `ui/manifest.json` integrity. The emitted bundle imports without `process.env` leaks, the manifest hash matches `shasum -a 256`, the happy-dom bundle smoke verifies all four light-DOM widgets plus detail route params, and `vue-tsc --noEmit -p extensions/first-party/ext_pull_requests/ui/tsconfig.json`, `bun run typecheck:v3`, `bun run build:v3`, root `bun run typecheck`, root `bun run build`, and `git diff --check` pass.*
- [x] `ext_checks`: same - *ported the existing checks board/detail placeholders to Vue SFCs, registered `comtrya-checks-board` and `comtrya-checks-detail` through `defineExtensionWidget`, preserved the default `setup(host)` `repository.checks` slot and `/` route, and emitted `extensions/first-party/ext_checks/assets/index.js` through `extensions/bundler/build-extension.sh extensions/first-party/ext_checks` with refreshed `ui/manifest.json` integrity. The emitted bundle imports without `process.env` leaks, the manifest hash matches `shasum -a 256`, the happy-dom bundle smoke verifies both light-DOM widgets, and `vue-tsc --noEmit -p extensions/first-party/ext_checks/ui/tsconfig.json`, `bun run typecheck:v3`, `bun run build:v3`, root `bun run typecheck`, root `bun run build`, and `git diff --check` pass.*
- [x] `ext_workspace_home`: layout-only — register slot consumers - *ported the workspace-home slot consumers to Vue SFCs and registered `comtrya-home-your-work`, `comtrya-home-repositories`, `comtrya-home-activity`, and `comtrya-home-instance` through `defineExtensionWidget`; the default `setup(host)` now preserves the old `home.*` slots and also registers the shell-v3 `workspace.home.top/left/center/right` slots, with server/frontend manifest validation updated to accept those names. `extensions/bundler/build-extension.sh extensions/first-party/ext_workspace_home` emits `extensions/first-party/ext_workspace_home/assets/index.js` and refreshes `ui/manifest.json` integrity; the emitted bundle imports without `process.env` leaks, the manifest hash matches `shasum -a 256`, the happy-dom bundle smoke verifies old+new slot registrations and all four light-DOM widgets, and `vue-tsc --noEmit -p extensions/first-party/ext_workspace_home/ui/tsconfig.json`, `bun run typecheck`, `bun run typecheck:v3`, `bun run build:v3`, root `bun run build`, `cargo test -p comtrya-server manifest_v2`, and `git diff --check` pass.*
- [x] Shell loads each extension's UI bundle at startup keyed off its manifest - *shell-v3 startup now queries `extensionInstallations`, fetches each `/_extensions/<id>/manifest.json`, parses the UI manifest, dynamically imports `assets.entry`, and runs each bundle's default `setup(host)` into the SDK slot/card/route registries. The loader provides `comtryaClient`, `viewer`, and `capabilities` to mounted slot and route elements, and Vite dev now proxies `/_extensions` and `/readyz` to the kernel for extension assets/widgets. `bun run typecheck:v3`, `bun run build:v3`, root `bun run typecheck`, root `bun run build`, and `git diff --check` pass.*
- [x] Browser-test: issue list renders from real WASM-backed data; close button fires `closeIssue`; UI updates optimistically via `applyOptimistic` - *Fresh production-testbed backend ran from `/private/tmp/comtrya-m8-vue-smoke.lTMK6T` on `127.0.0.1:8081`; shell-v3 Vite ran on `127.0.0.1:5175` and loaded `ext_issues` from its live UI manifest. A live GraphQL `issues.create` call created rawkode/rawkode issue `iss_00000000000S8VTAA5M6GG0004` (#2) through the WASM-backed path. Headless Chrome/CDP verified `/r/rawkode/rawkode` rendered `comtrya-issues-list` in the `repository.issues` slot with the issue card from `issues.list`; `/x/issues/ws_01HV0K4XAVE2H6R5M8KJZ8Q1A3/2` rendered the close button; CDP paused the `issues.close` GraphQL request before the server reply and still observed the UI switch to `closed` plus disabled `Reopen issue`, proving the `applyOptimistic` path. After continuing the request, `issues.byNumber` returned `CLOSED`/`completed`, and extension storage events included `emitterExtension="ext_issues"` + `eventType="dev.comtrya.issues.closed"` for the same issue. `vue-tsc --noEmit -p extensions/first-party/ext_issues/ui/tsconfig.json`, `bun run typecheck:v3`, `bun run build:v3`, root `bun run typecheck`, root `bun run build`, manifest hash check, emitted-bundle `process.env` leak check, happy-dom optimistic close smoke, and `git diff --check` pass.*

## M9 — Vue shell at parity with Astro
- [x] Port `/` (root / workspace home) from Astro to Vue - *`frontend/shell-v3/src/routes/WorkspaceHome.vue` now queries live GraphQL workspace/instance/extension data, renders the Astro-era `home-shell`, `home-new-repo-cta`, `home-repositories`, and `home-repositories-new` smoke surfaces, links real repositories to `/r/<path>`, and keeps all four `workspace.home.*` slot frames mounted. `frontend/shell-v3/src/App.vue` now refreshes the shared shell chrome from live workspace data, so the sidebar matches the root page instead of the old static `Comtrya / 1 repository` fallback. Live Chrome/CDP against the production-testbed backend at `127.0.0.1:8081` and shell-v3 Vite at `127.0.0.1:5175` verified `/` renders `Comtrya Labs`, summary `2` repositories / `4` extensions / runtime `enabled`, repository links for `comtrya/comtrya` and `rawkode/rawkode`, live sidebar `Comtrya Labs` / `2 repositories`, and all `workspace.home.top|left|center|right` slot mounts. `bun run typecheck:v3`, `bun run build:v3`, root `bun run typecheck`, root `bun run build`, and `git diff --check` pass.*
- [x] Port `/r/<...>` (repo home) from Astro to Vue - *`RepoHome.vue` now resolves live repository metadata through GraphQL, renders the Astro-era `repo-dashboard` surface, repo tabs, live description/visibility/open-PR metadata, and a slot stack with stable `#overview`, `#code`, `#issues`, and `#checks` anchors. Shell-v3 now defines a native `comtrya-core-code-browser` and registers it as the core `repository.code` slot contribution alongside the core repository summary. Live Chrome/CDP against `/r/comtrya/comtrya` on the production-testbed backend verified the header path `comtrya/comtrya`, summary path/default branch/repo id, repo tabs, `repository.overview` children `comtrya-repository-summary` + `comtrya-pulls-overview`, `repository.code` child `comtrya-core-code-browser` with `repo-code-section`, `repo-code-tree`, and README preview, plus `repository.issues` / `repository.checks` mounting `comtrya-issues-list` and `comtrya-checks-board`. `bun run typecheck:v3`, `bun run build:v3`, root `bun run typecheck`, root `bun run build`, and `git diff --check` pass.*
- [x] Port `/x/<prefix>/...` (extension routes) from Astro to Vue - *`ExtensionRoute.vue` now exposes the Astro-era `extension-page` smoke surface while preserving shell-v3 route matching, `routeParams`, and mounted route-element context. Live Chrome/CDP against the production-testbed shell verified `/x/issues/` resolves to `comtrya-issues-list` with route summary `issues` / `/` / `comtrya-issues-list` and renders live issue cards from `issues.list`; `/x/issues/ws_01HV0K4XAVE2H6R5M8KJZ8Q1A3/2` resolves to `comtrya-issue-detail`, forwards params, renders issue `iss_00000000000S8VTAA5M6GG0004`, and shows the `Reopen issue` action for the closed WASM-backed issue. `bun run typecheck:v3`, `bun run build:v3`, root `bun run typecheck`, root `bun run build`, and `git diff --check` pass.*
- [x] Port settings / health / admin pages from Astro to Vue - *Added shell-v3 routes for `/new`, `/instance`, `/settings`, and `/health`, plus `/healthz` Vite proxying. `/new` now renders the old `new-repo-shell` / `new-repo-header` / form surface in Vue and submits `createRepository` through GraphQL. `/instance`, `/settings`, and `/health` share a Vue instance-health page that reads live `/readyz`, `/healthz`, and GraphQL instance data, then renders readiness checks, capabilities, extension counts, and unsupported surfaces. Live Chrome/CDP verified `/new` exposes the form with path/import inputs and `Create repository`, `/instance` reports ready `yes`, health `ok`, mode `production-testbed`, instance `Comtrya Production Testbed`, and `legacy_v1_api`, while `/settings` and `/health` render the same live readiness data under their own routes. `bun run typecheck:v3`, `bun run build:v3`, root `bun run typecheck`, root `bun run build`, and `git diff --check` pass.*
- [x] Implement SSR (if chosen) or wire SPA fallback for routes Astro served statically - *The pre-flight decision is SPA, so `shell-v3/vite.config.ts` now sets `appType: "spa"` explicitly and shares the kernel proxy table between dev and preview, including `/graphql`, `/_extensions`, `/readyz`, `/healthz`, and events. `bun run preview:v3 -- --host 127.0.0.1 --port 5176` against the live backend returned HTTP 200 for `/new`, `/r/rawkode/rawkode`, `/x/issues/`, and `/health`, served the built Vue shell HTML for deep routes, and proxied backend JSON for `/readyz` (`ready=true`) plus `/healthz` (`status=ok`). `bun run typecheck:v3`, `bun run build:v3`, root `bun run typecheck`, root `bun run build`, and `git diff --check` pass.*
- [x] Update every URL in `start.sh` smoke checks to assert against the Vue shell - *`start.sh` now builds and previews `frontend` through `build:v3` / `preview:v3`, verifies the Vue shell HTML and asset bundle for static routes, routes `/api/v1`, `/auth`, `/git`, GraphQL, health, readyz, events, and extension assets through the shell-v3 proxy, and rewrites the browser repo-dashboard smoke to assert the Vue slot mounts instead of Astro body data.*
