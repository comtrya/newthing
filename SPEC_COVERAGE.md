...
- `start.sh` seeds demo input into versioned extension storage document tables, opens and validates a real local bare Git repository, prints live repository/branch/extension seed evidence from GraphQL, runs the Astro frontend in front of the Rust server, and verifies operator-code exchange, GraphQL live Git data, SSR-rendered head OID equality, GraphQL diff equality with cloned Git output, single-use and expired event sessions, extension manifest/asset sessions, typed Wasmtime resolver summaries, mounted non-empty extension surfaces, Git clone/fetch, and receive-pack fail-closed behavior through that frontend origin.
...
- The Astro repository UI renders live refs, branches, commits, and Git/storage-derived metric counts from the server; code browsing, pull requests, and checks are composed through extension slots, and smoke executes the served first-party assets to assert non-empty mounted output for those surfaces.
