...
- Rust workspace tests pass: 77 core unit tests, 1 MVP integration test, and 27 server route/runtime tests.
...
- `start.sh` seeds demo input into versioned extension storage document tables, opens and validates a real local bare Git repository, prints live repository/branch/extension seed evidence from GraphQL, runs the Astro frontend in front of the Rust server, and verifies operator-code exchange, GraphQL live Git data, SSR-rendered head OID equality, GraphQL diff equality with cloned Git output, single-use and expired event sessions, extension manifest/asset sessions, typed Wasmtime resolver summaries, Git clone/fetch, and receive-pack fail-closed behavior through that frontend origin.
...
- The frontend discovers installed extensions from GraphQL, validates their UI manifests, imports their entry assets, creates extension host elements from manifest slot declarations, and renders visible load/resolver/permission error states.
