...
var po, mo;
async function ho(e = {}) {
	return po && po.expiresAtMs > Date.now() + 5e3 ? po.token : (mo ||= _o(e).finally(() => {
		mo = void 0;
	}), mo);
}
function go() {
...
async function _o(e) {
	let t = e.operatorCode ?? vo(), n = e.fetchImpl ?? fetch, r = e.baseUrl ?? "";
	if (!t) return;
...
function So(e) {
	let t = async (t, n) => {
		let r = await ho(), i = { "Content-Type": "application/json" };
...
var To = [
	...
	"Alt",
	"Control"
], Eo = typeof navigator == "object" ? navigator.platform : "", Do = /Mac|iPod|iPhone|iPad/.test(Eo), Oo = Do ? "Meta" : "Control", ko = Eo === "Win32" ? ["Control", "Alt"] : Do ? ["Alt"] : [];
function Ao(e, t) {
	return typeof e.getModifierState == "function" && (e.getModifierState(t) || ko.includes(t) && e.getModifierState("AltGraph"));
}
function jo(e) {
...
function Mo(e, t) {
	...
	return !((r instanceof RegExp ? !r.test(e.key) && !r.test(e.code) : r.toUpperCase() !== e.key.toUpperCase() && r !== e.code) || n.find(function(t) {
		...
		return !n.includes(t) && r !== t && Ao(e, t);
	}));
}
function No(e, t) {
	t === void 0 && (t = {});
	...
	return function(e) {
		e instanceof KeyboardEvent && (r.forEach(function(t) {
			...
			Mo(e, a[0]) ? a.length > 1 ? i.set(n, a.slice(1)) : (i.delete(n), r(e)) : Ao(e, e.key) || i.delete(n);
		}), a && clearTimeout(a), a = setTimeout(i.clear.bind(i), n));
	};
}
function Po(e, t, n) {
	var r = n === void 0 ? {} : n, i = r.event, a = i === void 0 ? "keydown" : i, o = r.capture, s = No(t, { timeout: r.timeout });
...
function Bo(e) {
	return e.replace(/[&<>"']/g, (e) => zo[e] ?? e);
}
var Vo = /\bhttps?:\/\/[^\s<]+[^\s<.,;:!?)]/g, Ho = "CODE", Uo = "END";
function Wo(e, t) {
	let n = [], r = e.replace(/`([^`]+)`/g, (e, t) => (n.push("<code>" + t + "</code>"), Ho + (n.length - 1) + Uo));
	if (r = r.replace(Vo, (e) => "<a href=\"" + e + "\" rel=\"noopener noreferrer\">" + e + "</a>"), t.workspaceId) {
...
function Ko(e, t = {}) {
	...
	for (let e of n) switch (e.kind) {
		case "heading": {
			let n = e.level ?? 1, i = Wo(Bo(e.text), t);
			r.push("<h" + n + ">" + i + "</h" + n + ">");
			break;
		}
		case "paragraph": {
			let n = Wo(Bo(e.text), t);
			r.push("<p>" + n.replace(/\n/g, "<br />") + "</p>");
			break;
		}
		case "code": {
			let t = e.lang ? " data-lang=\"" + Bo(e.lang) + "\"" : "";
			r.push("<pre" + t + "><code>" + Bo(e.text) + "</code></pre>");
			break;
		}
		case "list": {
			let n = e.ordered ? "ol" : "ul", i = (e.items ?? []).map((e) => "  <li>" + Wo(Bo(e), t) + "</li>").join("\n");
			r.push("<" + n + ">\n" + i + "\n</" + n + ">");
			break;
		}
	}
...
function Jo(e) {
	...
	for (let [n, r] of Object.entries(e.propertyAliases ?? {})) Object.defineProperty(t.prototype, n, {
		...
		set(e) {
			this[r] = e, typeof e == "string" && this.setAttribute(Zo(r), e);
		}
	});
...
var Qo = {
...
}, $o = { class: "docs-head" }, es = { class: "title-block" }, ts = { class: "muted" }, ns = {
	key: 0,
	class: "muted error",
	role: "alert"
}, rs = {
	...
	class: "muted error",
	role: "alert"
}, is = { class: "docs-project-head" }, as = { class: "docs-project-root" }, os = { class: "docs-type-head" }, ss = { class: "docs-type-key" }, cs = { class: "docs-type-label" }, ls = { class: "docs-type-scope" }, us = { class: "muted docs-type-count" }, ds = {
	key: 0,
	...
	"onMouseenter",
	"onKeydown"
], hs = { class: "docs-file-head" }, gs = { class: "docs-file-caret" }, _s = { class: "docs-file-title" }, vs = { class: "docs-file-path" }, ys = {
	key: 0,
	...
	setup(e) {
		...
		return (t, a) => (J(), Y("section", Qo, [
			X("header", $o, [X("div", es, [a[8] ||= X("h2", null, "Docs", -1), X("span", ts, [n.value === "loading" ? (J(), Y(K, { key: 0 }, [Q("reading repo CUE config…")], 64)) : n.value === "error" ? (J(), Y(K, { key: 1 }, [Q("unavailable")], 64)) : c.value === 0 ? (J(), Y(K, { key: 2 }, [
			...
			n.value === "error" ? (J(), Y("p", ns, A(r.value), 1)) : i.value?.error ? (J(), Y("p", rs, A(i.value.error), 1)) : Fi("", !0),
			(J(!0), Y(K, null, sr(s.value, (t) => Sn((J(), Y("article", {
				key: t.name,
				class: "docs-project"
			}, [X("header", is, [X("h3", null, A(t.name), 1), X("code", as, A(t.root || "<repo root>") + "/", 1)]), (J(!0), Y(K, null, sr(S(t), (n) => (J(), Y("section", {
				key: n.key,
				class: "docs-type"
			}, [
				X("header", os, [
					X("code", ss, A(n.key), 1),
					X("span", cs, A(n.type.label || n.key), 1),
					X("code", ls, A(v(t, n.type) || "<project root>") + "/", 1),
					X("span", us, [Q(A(x(t, n.type).length) + " file", 1), x(t, n.type).length === 1 ? Fi("", !0) : (J(), Y(K, { key: 0 }, [Q("s")], 64))])
				]),
				n.type.description ? (J(), Y("p", ds, A(n.type.description), 1)) : Fi("", !0),
				C(n.type).length > 0 ? (J(), Y("dl", fs, [
				...
				x(t, n.type).length > 0 ? (J(), Y("ol", ps, [(J(!0), Y(K, null, sr(x(t, n.type), (t) => (J(), Y("li", {
					key: t.path,
					class: he(["docs-file", {
					...
					X("header", hs, [
						X("span", gs, A(d(t.path) ? "▾" : "▸"), 1),
						X("strong", _s, A(t.title), 1),
						X("code", vs, A(t.path), 1)
					]),
					Object.keys(t.frontMatter).length > 0 ? (J(), Y("dl", ys, [(J(!0), Y(K, null, sr(t.frontMatter, (e, t) => (J(), Y(K, { key: t }, [X("dt", null, [X("code", null, A(t), 1)]), X("dd", null, A(T(e)), 1)], 64))), 128))])) : Fi("", !0),
					t.body && !d(t.path) ? (J(), Y("p", bs, A(Wt(qo)(t.body)), 1)) : Fi("", !0),
					t.body && d(t.path) ? (J(), Y("article", {
						key: 2,
						class: "docs-file-rendered",
						innerHTML: Wt(Ko)(t.body, { workspaceId: e.workspaceId ?? "" })
					}, null, 8, xs)) : Fi("", !0)
				], 42, ms))), 128))])) : (J(), Y("p", Ss, [
