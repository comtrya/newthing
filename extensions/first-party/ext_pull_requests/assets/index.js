...
async function Us(e) {
	if (!e) return [];
	let t = `comtrya://pull-request/${e}`, n = (await Ao().query("query LinkedIssues($from: ResourceURN!, $kind: ResourceURN) {\n      relations.outgoing(from: $from, kind: $kind) { id kind from to source target }\n    }", {
		from: t,
...
var hc = {
	...
	setup(e) {
		...
		}), b = $, x = Q(() => Ys(r.value?.state)), S = Q(() => r.value && (r.value.state === "READY" || r.value.state === "DRAFT")), C = Q(() => r.value && r.value.state !== "CLOSED" && r.value.state !== "MERGED"), w = Q(() => r.value?.bodyMarkdown ? vs(r.value.bodyMarkdown, { workspaceId: defaultWorkspaceId() }) : ""), ee = Q(() => r.value ? `comtrya://pull-request/${r.value.id}` : ""), T = /* @__PURE__ */ L(null);
		function te(e) {
			let t = e.detail;
