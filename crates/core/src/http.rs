...
pub fn allowed_methods_for_route(route: &str) -> Vec<&'static str> {
    match route {
        ...
        "/events/session" | "/auth/token-exchange" | "/forge-ui/session" => {
            vec!["POST", "OPTIONS"]
        }
        route if route.starts_with("/auth/") => vec!["GET", "POST", "OPTIONS"],
        route if route.starts_with("/forge-ui/") => vec!["GET", "OPTIONS"],
        ...
        // so it accepts the same method set the old `/git/` route did.
        route if route.starts_with("/r/") => vec!["GET", "POST", "OPTIONS"],
        _ => vec!["OPTIONS"],
    }
}
