...
mod tests {
    ...
    /// End-to-end `epics.link-issue`: dispatch through the real registry
    /// against the rebuilt ext_issues + ext_epics components. Verifies the
    /// participation gate (the issue's repo must have opted into BOTH
    /// ext_issues and ext_epics) and that a successful link round-trips
    /// through `issues-in-epic`. Guards the wasm-export contract: a stale
    /// dist/ext_epics.wasm without the `link-issue` export would fail at
    /// instantiation here.
    #[test]
    fn registry_dispatcher_links_issue_into_epic_with_participation_gate() {
        let issues_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../extensions/first-party/ext_issues");
        let epics_root = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
            .join("../../extensions/first-party/ext_epics");
        for (root, id) in [(&issues_root, "ext_issues"), (&epics_root, "ext_epics")] {
            if !root.join(format!("dist/{id}.wasm")).is_file() {
                eprintln!("SKIP link-issue e2e: build {id}");
                return;
            }
        }
        ...
        let registry = WasmRegistry::new().expect("build registry");
        registry
            .register_from_manifest(&issues_root)
            .expect("register ext_issues");
        registry
            .register_from_manifest(&epics_root)
            .expect("register ext_epics");
        registry
        ...
        registry.install_repo_enablement(StaticRepoEnablement::new([
            (repo, vec!["ext_issues", "ext_epics"]),
            // A repo that enabled issues but NOT epics: the gate must reject.
            (other_repo, vec!["ext_issues"]),
        ]));
        let tmp = tempdir_for_test("comtrya-link-issue-e2e");
        let store =
            Arc::new(crate::ExtensionRuntimeStore::open_for_tests(&tmp).expect("open ext store"));
        let dispatcher = RegistryDispatcher {
            registry,
        ...
        let principal = "comtrya://user/usr_link_test";

        // Open an issue (in the fully-enabled repo) and create an epic.
        ...
        .unwrap();
        let issue_ref = issue.get("ref").and_then(Value::as_str).map_or_else(
            || format!("comtrya://issue/{}", issue["id"].as_str().unwrap()),
            str::to_string,
        );

        let epic: Value = serde_json::from_slice(
            &dispatcher
                .dispatch(
                    "ext_epics",
                    "epics.create-epic",
                    &serde_json::to_vec(&serde_json::json!({
                        "workspace": "comtrya://workspace/ws_link",
                        "title": "holder epic",
                        "bodyMarkdown": "body",
                        "labels": [],
                    }))
        ...
        .unwrap();
        let epic_ref = epic.get("ref").and_then(Value::as_str).map_or_else(
            || format!("comtrya://epic/{}", epic["id"].as_str().unwrap()),
            str::to_string,
        );

        // Happy path: the issue's repo enabled both extensions → link
        // succeeds and the issue shows up as a member of the epic.
        let members: Value = serde_json::from_slice(
            &dispatcher
                .dispatch(
                    "ext_epics",
                    "epics.link-issue",
        ...
        assert!(
            members
                ...
                .unwrap()
                .iter()
                .any(|m| m.as_str() == Some(issue_ref.as_str())),
            "linked issue must appear in epic members: {members}"
        );

        // Idempotent: linking the same pair again returns the same single
        // member, not a duplicate or an error.
        let again: Value = serde_json::from_slice(
            &dispatcher
                ...
                .expect("re-link is idempotent"),
        )
        .unwrap();
        assert_eq!(again.as_array().unwrap().len(), 1, "re-link must not dup");

        // Gate: an issue whose repo did NOT enable ext_epics cannot be
        // linked, even though the epic is instance-scoped.
        let ungated: Value = serde_json::from_slice(
            &dispatcher
                .dispatch(
                    "ext_issues",
                    "issues.open-issue",
                ...
                .expect("open ungated issue"),
        )
        .unwrap();
        let ungated_ref = ungated.get("ref").and_then(Value::as_str).map_or_else(
            || format!("comtrya://issue/{}", ungated["id"].as_str().unwrap()),
            str::to_string,
        );
        let denied = dispatcher
            ...
            .expect_err("issue from a non-epics repo must be rejected");
