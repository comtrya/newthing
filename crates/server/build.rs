...
fn main() {
    ...
    // Compose dispatch_table.rs — pulls every per-extension file in
    // via include!() and emits a top-level dispatch_route().
    ...
    for (ext_id, path) in &per_extension_files {
        ...
        // paths. We embed the absolute path; it's the build-time
        // OUT_DIR location and stable for the build.
    ...
    table.push_str("    None\n}\n\n");
    // Re-export DispatchInfo from the first per-extension module so
    // call sites don't have to know which ext defined it. They're all
    // structurally identical (generated from the same render fn).
    if let Some((first_ext, _)) = all_routes.first() {
        table.push_str(&format!(
            "pub use ext_{}::DispatchInfo;\n",
            safe_ident(first_ext)
        ));
    } else {
        // No extensions yet — emit a stub so includes still compile.
