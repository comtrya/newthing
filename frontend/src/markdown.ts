/**
 * Tiny markdown renderer for repo + doc bodies.
 *
 * Handles a small subset — headings, paragraphs, fenced code,
 * lists, inline code/bold/italic — escapes everything else as
 * text. Designed for a remark + rehype + shiki upgrade later
 * without changing the call site.
 *
 * Mirrors the renderer in `ext_docs/ui/src/markdown.ts` and
 * `ext_epics/ui/src/markdown.ts`; deliberately duplicated into
 * the shell so it can be used in RepoHome (and future shell
 * surfaces) without crossing the extension boundary.
 */
