# Comtrya local runtime runbook

> **URL scheme (v3):** repositories at `/r/<group>/<...>/<repo>`.
> Extension-owned pages live at `/x/<prefix>/<...>`. The workspace homepage is
> `/`.

This runbook is for the local production-testbed runtime.

## Clean Reset

Use a reset when you want an empty runtime data directory before the smoke creates
its own test data through product APIs:
...
- Imported smoke repository: `http://127.0.0.1:4321/r/comtrya/comtrya`
...
`.envrc.example` provides the local operator code:
...
- GraphQL proving the fresh workspace starts empty
- `createRepository` importing a temporary Git repository generated by the smoke
...
- `repositories/<owner>/<repo>.git`: bare Git repositories created/imported through GraphQL.
